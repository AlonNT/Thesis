...
class IntrinsicDimensionCalculator(Callback):
    ...
    def calc_int_dim_per_layer_on_dataloader(self, trainer, pl_module, dataloader, name: str = ''):
        ...
        for i in range(len(pl_module.features)):
            ...
            estimates = get_estimates_matrix(patches, self.k3 if self.plot_graphs else self.k2)
            estimate_mean_over_data_points = torch.mean(estimates, dim=0)
...
def initialize(args: Args):
    ...
    intrinsic_dimension_calculator = IntrinsicDimensionCalculator(args.arch.n_patches, args.arch.k1, args.arch.k2,
